    ...
    for step in range(1,epoch+1, 1):
        ...
        for i in range(len(images)):
            ...
            for d in range(10):
                ...
                for x in range(image_rows):
                    for y in range(image_columns):
                        result += (digits[d].weight)[x][y]*data[x][y]
            ...
            if bestLabel != trueLabel:
                ...
                for x in range(image_rows):
                    for y in range(image_columns):
                        trueWX = (digits[trueLabel].weight)[x][y]*data[x][y]
                        bestWX = (digits[bestLabel].weight)[x][y]*data[x][y]
                        (digits[trueLabel].weight)[x][y] += alpha*sigmoid(trueWX)*(1-sigmoid(trueWX))*data[x][y]
                        (digits[bestLabel].weight)[x][y] += -alpha*sigmoid(bestWX)*(1-sigmoid(bestWX))*data[x][y]
